...
$patientIdQuery = "SELECT DISTINCT pat_id from ans_post";
...
$formDateResult = mysql_query("SELECT DISTINCT dateof FROM ans_post ORDER BY dateof ASC"); //we pull all the dates from the table so that we can use it to figure out the type field for each object
$dateArray = array();
$typeVal = 1;
...
foreach ($patIdArray as $id) {
    $query = "SELECT * FROM ans_post WHERE pat_id = " . $id;
    ...
    //insert first record manually, setting some data once
    $dbObj = new Post($row['pat_id']);
    $dbObj->setDateOfByTimeStamp($row['dateof']);
    $dbObj->setSurId($row['sur_id']);
    $dbObj->setDateOfExamByTimeStamp($row['dateofexam']);
    $dbObj->setDosePainMedUsed($row['dosepainmedused']);
    $dbObj->setPainMedUsed($row['painmedused']);

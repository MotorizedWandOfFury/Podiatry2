...
$patientIdQuery = "SELECT DISTINCT pat_id from ans_xrays";
...
$formDateResult = mysql_query("SELECT DISTINCT dateof FROM ans_xrays ORDER BY dateof ASC"); //we pull all the dates from the table so that we can use it to figure out the type field for each object
$dateArray = array();
$typeVal = 1;
...
foreach ($patIdArray as $id) {
    $query = "SELECT * FROM ans_xrays WHERE pat_id = " . $id;

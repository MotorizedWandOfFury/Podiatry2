...
if ($admin->isLogged() == FALSE)
    $error->doGo("index.php", "Only for Administrators.");

if ($_GET['SUBMIT'] == FALSE)
{
    // Load the HTML, CSS.
    echo $layout->loadCSS("Physician Page", "../");
    // Load the body.
    echo $layout->startBody();

    echo "<h3>Add SF-36 Results</h3>";
    ...
    echo "
            <form action='" . $_SERVER['SCRIPT_NAME'] . "?id=" . $pat->GetId() . " method='post'>
                <div class='container'>
                    <div class='greybox'>
                                    <p>1) Patient: " . $pat->GetFullName() . "&nbsp;&nbsp;&nbsp;
                                       ...
                                       3) Extremity: " . $pat->GetExtremity() . "
                                       <input type='hidden' name='id' value='" . $pat->GetId() . "'>
                                    </p>
	";
...
else if ($_GET['SUBMIT'] != FALSE)
{
    ...
    $dateof = mktime(0, 0, 0, $clean->toInt($_GET['M']), $clean->toInt($_GET['D']), $clean->toInt($_GET['Y']));
